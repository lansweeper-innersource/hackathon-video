...
project.json      revid.ai project definition (slides, transcripts, word-level timestamps)
...
The source material comes from a [revid.ai](https://revid.ai) project. revid.ai renders videos client-side in the browser from a JSON project definition, so there is no pre-rendered MP4 to download. This script reconstructs the video offline:
